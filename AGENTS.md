...
- **When a skill owes a payload version stamp**: only when it **copies artifacts into the target repo** that can then drift from what the skill ships. Such a skill owns `scripts/payload.version` as the single source of truth and writes it on install — a `.version` file at the payload root, or a marker comment when the assets scatter and have no root. A skill whose install is regenerated on every run does not drift and needs no stamp. The comparison belongs in `verify-*.sh`, which runs from the skill directory and can reach both halves; a session hook runs inside the target repo, where the skill directory is unreachable, so it can never make it. The harness fixture boards mirror that payload too — `scripts/sync-fixture-boards.sh --check` holds them in step and runs at pre-commit and in CI, so a payload bump cannot land without its fixture refresh. The reverse is a separate gate, because nothing else can see it: [`scripts/verify-payload-version.sh`](scripts/verify-payload-version.sh) refuses a payload edit whose `payload.version` does not move, at pre-commit on the staged set and in CI across the branch. It exists because the fixture check cannot catch this — no fixture carries the installed CLI (the harness points `HANDOFF_BIN` at the payload), so a CLI-only edit drifts nothing, passes every other gate, and ships at the old stamp with no install ever told it is behind. That happened once and had to be corrected after the fact. `*.selftest.sh` is excluded, since a suite is never installed.
...
| `engineering`  | `setup-secret-guard`          | `beta`         | Install the secret guard — a redacting read-path guard plus the shared detection and masking engine (`secret-scan`, `redact-view`) — into the `.claude` cascade, so a credential file stays usable while its values are replaced by stable fingerprints. Chains after `initial-project`.                                                                                                                                                                                                          |
| `engineering`  | `setup-handoff`               | `beta`         | Install a lease-based handoff coordination protocol (`.agents/handoff/`) so multiple agents/sessions/repos work the same code without clobbering — claim/release leases, per-tool enforcement hooks (user picks a primary), and legacy-install migration. Detects boards in the repo and two parent levels, records a developer's own board in `handoff.local.json`, and suggests ignore rules. Restores held-lease context after compaction on Claude Code only. Chains after `initial-project`. |
| `engineering`  | `run-handoff`                 | `beta`         | The claim → work → release discipline over an installed handoff board: check the board, claim before editing, and release with an honest status (`done` requires evidence, `--for-review` hands work back without closing it); `checkpoint` progress, `move` a handoff between boards, and link its ticket with `--ref`; size bundle children as slices, record `## Ruled out`, keep solo continuity off the board. Chains after `setup-handoff`.                                                 |
| `engineering`  | `delegate-handoff`            | `beta`         | Judgment for handing a handoff to someone outside the board — is it brief-able, how to review what comes back, and whether a human will be watching (`--executed-by hitl\|afk`, which decides what evidence the closure owes). Drives `handoff export`/`handoff import --result`, including a bundle exported as a parent issue with one issue per child; import never sets `status`. Ships markdown only. Chains after `run-handoff`.                                                            |
| `engineering`  | `repair-handoff`              | `beta`         | Smoke-test the handoff CLI, then re-check, validate, and repair the board wiring and board state (index drift, orphaned leases, doc frontmatter, section resolution, orphaned delegations). Chains after `setup-handoff`.                                                                                                                                                                                                                                                                         |
| `engineering`  | `register-cross-repo-handoff` | `beta`         | Declare groups of peer repos in a per-workspace `.agents/handoff.json` cascade (opt-in user → workspace → subdir), then sync to scaffold a dedicated board owned by no repo and wire every member to its own sub-indexed section (subfolder or prefix layout). No seed. Chains after `setup-handoff`.                                                                                                                                                                                             |
| `productivity` | `release-announcement`        | `beta`         | Turn a tagged version and its changelog into a user-facing announcement shaped for its channel (GitHub release, Slack, email), leading with user impact rather than the file diff. Can emit a second language.                                                                                                                                                                                                                                                                                    |
...
- **Standalone repo — never reference another project.** This repo is the _source_ of skills, not a
  ...
  this repo into a dedicated board, graph, or workspace owned elsewhere — install those _from_ here,

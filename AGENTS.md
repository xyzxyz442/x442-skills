...
| `engineering`  | `register-cross-repo-handoff` | `experimental` | Declare groups of peer repos in a per-workspace `.agents/handoff.json` cascade (opt-in user → workspace → subdir), then sync to scaffold a standalone shared board owned by no repo and wire every member to its own sub-indexed section (subfolder or prefix layout). No seed. Chains after `setup-handoff`.                                                                                                                                                                                     |

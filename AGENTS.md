...
| `engineering`  | `run-handoff`                 | `experimental` | The claim → work → release discipline over an installed handoff board: check the board, claim before editing, and release with an honest status (`done` requires evidence); `checkpoint` progress, `move` a handoff between boards, and link its ticket with `--ref`; size bundle children as slices, record `## Ruled out`, keep solo continuity off the board. Chains after `setup-handoff`.                                                                                                    |

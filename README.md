...
| [`register-cross-repo-handoff`](skills/engineering/register-cross-repo-handoff/SKILL.md) | `experimental` | Stands up a multi-group fleet from a `.handoff-repos.json` cascade: a standalone shared board owned by no repo, each member wired to its own sub-indexed section.                                                                  |

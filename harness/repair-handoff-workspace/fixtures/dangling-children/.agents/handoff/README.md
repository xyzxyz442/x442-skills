...
- `./handoff mirror [--dry-run]` projects this section's open work one way into the board's issue
  ...
  handoff through an issue and bring the reply back for review (ADR 0011). A **public** repository
  is published to only when `external.allowPublic: true` is set on the board AND the doc carries
  `share: public` (ADR 0013); visibility is checked on every run, and unknown counts as public.
...
| `share`                                        | _(optional)_ `public` marks a doc whose audience may be the public. Honoured only when the board sets `external.allowPublic`; on a private tracker it changes nothing. Set at `new --share public`; refused with `restricted` (restricted wins) and on standalone docs; `migrate` never backfills it (ADR 0013).                                                                                                                                                                                                                                                                                                                                                                                                                         |

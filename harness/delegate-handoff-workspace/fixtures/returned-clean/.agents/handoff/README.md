...
| `review` / `executed_by`                       | `review: pending` has two writers. `import --result` sets it alongside `executed_by: delegate`, for work done off the board; `release --for-review` sets it for a collaborator who is on the board. While a review is pending, `release --status done` compares `--verified-by` against the executor's own account of the work — the delegate's reported Result block, or, where there is none, the doc's `## Current state`. A word-for-word match is **refused** for delegated work (that is the delegate reviewing itself) and **warned** otherwise. Re-run the check and say what you observed, or release `open` and hand it to a reviewer. |

...
drift_file() { # -> this section's generated tracker-drift report (ADR 0014)
  case "$LAYOUT" in
    subfolder) [ -n "$GROUP" ] && {
      printf '%s' "$DIR/$GROUP/TRACKER-DRIFT.md"
      return
    } ;;
    prefix) [ -n "$GROUP" ] && {
      printf '%s' "$DIR/TRACKER-DRIFT-$GROUP.md"
      return
    } ;;
  esac
  printf '%s' "$DIR/TRACKER-DRIFT.md"
}
# The drift report's table rows, read with sed alone: this runs on every session start and must not
# be the reason one is slow. Absent file — the normal, quiet state — means no drift and no cost.
drift_rows() { # -> id<TAB>number<TAB>board-status<TAB>tracker-state
  local out
  out="$(drift_file)"
  [ -f "$out" ] || return 0
...
doc_id_of() {
  ...
  # pretool handler can deny hand-edits of any generated index. The tracker-drift report (ADR 0014)
  # is generated the same way and gated for the same reason.
  case "$p" in
    "$DIR"/INDEX.md | "$DIR"/INDEX-*.md | "$DIR"/*/INDEX.md)
      printf 'INDEX'
      return 0
      ;;
    "$DIR"/TRACKER-DRIFT.md | "$DIR"/TRACKER-DRIFT-*.md | "$DIR"/*/TRACKER-DRIFT.md)
      printf 'TRACKER-DRIFT'
      return 0
      ;;
...
case "$KIND" in
  ...
  sessionstart)
    ...
    out=""
    refs=""
    # Read once for the whole banner: the report is one small generated file and every doc consults it.
    DRIFT="$(drift_rows)"
    while IFS= read -r f; do
      [ -n "$f" ] || continue
      id="$(basename "$f" .md)"
      # Tracker drift (ADR 0014) — the issue says finished and the board does not. Surfaced HERE
      # because this banner is where an agent decides what to pick up, and picking up work a
      # teammate has already closed out there is exactly the waste the report exists to prevent.
      # It is a note, never a status: `done` still needs evidence on the board.
      drift_n=""
      [ -n "$DRIFT" ] && drift_n="$(printf '%s\n' "$DRIFT" | awk -F'\t' -v i="$(meta "$f" id)" -v b="$id" \
        '$1 == i || $1 == b { printf " [⚠️ tracker drift — issue #%s is %s out there; status changes on the board, with evidence]", $2, $4; exit }')"
      ...
      case "$(meta "$f" type)" in
        orchestrator)
          ...
          refs="${refs}- ${id} — $(meta "$f" title)$(bundle_progress "$f")${drift_n}"$'\n'
          continue
          ;;
        standalone)
      ...
      line="- ${id} — $(meta "$f" status) · $(meta "$f" severity) · $(meta "$f" title)${drift_n}"
...
Re-run setup-handoff to update it, or fix config.json if it exists but is malformed."
  ...
  pretool-edit)
    ...
    [ "$id" = "TRACKER-DRIFT" ] && deny "TRACKER-DRIFT.md is generated by \`handoff mirror\` — never hand-edit it. Drift clears when the board and the tracker agree: close the handoff with evidence (\`${hd}/handoff release <id> --status done --verified-by \"…\"\`), or reopen the issue in the tracker. The next mirror run rewrites the file."

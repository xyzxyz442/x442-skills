...
Fixtures are inputs the harness feeds to a skill, not code this repo maintains, so
[.lintstagedrc.json](../.lintstagedrc.json) excludes `harness/**/fixtures/**` from prettier
and eslint. Two reasons, and both bite silently if you undo it: formatting would rewrite the
exact bytes a grader asserts on, and fixtures are _deliberately_ imperfect — a fixture's whole
job can be to start out unwired, so an autofix destroys the case it exists to test.

...
It is modeled on the mature harness in a prior skills repo, adapted to this repo's conventions
(kebab-case skill names, no vendor-specific prefix, and skill directories that hold only `SKILL.md`
plus `references/`).
...
## What this keeps from the prior harness, and what it changes
...
- **`harness/lib/`** holds the shared grader helpers and the aggregator/reorg scripts. The prior harness copied
...
- **Repo-native naming** — `initial-project-workspace`, no vendor-specific prefix; eval ids are kebab-case.

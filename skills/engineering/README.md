...
| `setup-secret-guard`          | `experimental` | Install the secret guard so credential values never reach a transcript — a `PreToolUse` guard that rewrites a plain read into a masked one, deny rules for the tools a hook cannot filter, and the shared engine both use. The home layer is load-bearing, so one install covers every repo. Chains after `initial-project`.                                  |
...
| `delegate-handoff`            | `experimental` | Judgment for handing a handoff to someone outside the board — is it brief-able, and how to review what comes back. Drives `handoff export` / `handoff import --result`; import never sets `status`. Chains after `run-handoff`.                                                                                                                               |
| `repair-handoff`              | `experimental` | Smoke-test the installed `handoff` CLI, then re-check, validate, and repair the board wiring and the board state a re-install cannot touch — index drift, orphaned leases, doc frontmatter, section resolution, orphaned delegations. Chains after `setup-handoff`.                                                                                           |
| `register-cross-repo-handoff` | `experimental` | Declare groups of peer repos in a per-workspace `.agents/handoff.json` cascade, then sync to scaffold a standalone shared board owned by no repo and wire every member to its own sub-indexed section (subfolder or prefix layout). No seed. Chains after `setup-handoff`.                                                                                    |

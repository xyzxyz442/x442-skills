...
| `setup-secret-guard`          | `beta`         | Install the secret guard so credential values never reach a transcript — a `PreToolUse` guard that rewrites a plain read into a masked one, deny rules for the tools a hook cannot filter, and the shared engine both use. The home layer is load-bearing, so one install covers every repo. Chains after `initial-project`.                                  |
| `setup-handoff`               | `beta`         | Install a lease-based handoff coordination protocol (`.agents/handoff/`) so multiple agents/sessions/repos work the same code without clobbering: atomic-lock claim/release, per-tool enforcement hooks (user picks a primary), fail-safe deny, self-maintaining leases, evidence-gated `done`, and legacy-install migration. Chains after `initial-project`. |
| `run-handoff`                 | `beta`         | The claim → work → release discipline over an installed handoff board: check the board, claim before editing, file handoffs, and release with an honest status. Chains after `setup-handoff`.                                                                                                                                                                 |
| `delegate-handoff`            | `beta`         | Judgment for handing a handoff to someone outside the board — is it brief-able, and how to review what comes back. Drives `handoff export` / `handoff import --result`; import never sets `status`. Chains after `run-handoff`.                                                                                                                               |
| `repair-handoff`              | `beta`         | Smoke-test the installed `handoff` CLI, then re-check, validate, and repair the board wiring and the board state a re-install cannot touch — index drift, orphaned leases, doc frontmatter, section resolution, orphaned delegations. Chains after `setup-handoff`.                                                                                           |
| `register-cross-repo-handoff` | `beta`         | Declare groups of peer repos in a per-workspace `.agents/handoff.json` cascade, then sync to scaffold a standalone shared board owned by no repo and wire every member to its own sub-indexed section (subfolder or prefix layout). No seed. Chains after `setup-handoff`.                                                                                    |

...
| `register-cross-repo-handoff` | `experimental` | Declare groups of peer repos in a per-workspace `.handoff-repos.json` cascade, then sync to scaffold a standalone shared board owned by no repo and wire every member to its own sub-indexed section (subfolder or prefix layout). No seed. Chains after `setup-handoff`.                                                                                     |

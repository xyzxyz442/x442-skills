#!/usr/bin/env python3
# splice-agents-block.py — splice the AGENTS.md knowledge-graph routing block into the target
# repo's AGENTS.md in place, touching not one byte outside the markers.
#
#   splice-agents-block.py --file <repo>/AGENTS.md --block assets/agents-knowledge-graph.md [--dry-run]
#
# Self-test:  python3 splice-agents-block.py --selftest
#
# This logic used to live as an inline python heredoc in SKILL.md — prose the agent executes. Prose
# cannot be unit-tested, cannot be linted, and had already diverged from its three siblings: it
# carried the separator defect below and nothing in the repo could have caught it. It is a file now
# so that it CAN carry the assertions. See agents-md-splice-audit-handoff.
#
...
# The block is declared "managed by setup-graph-hooks — do not edit between markers", so the
...
def splice(existing: str, block: str) -> str:
    ...
    # sibling skill's block below it — and setup-handoff splices exactly there in the documented
    # chain. Each re-run then flipped that line, so no repo wired for both was ever byte-stable.
    block = block.rstrip("\n") + "\n"
    return head + block + ("\n" + rest if rest else "")
...
def _selftest() -> int:
    ...
    The same seven cases setup-handoff's splice asserts, because the two splices are declared to
    mirror each other and the divergence between them is what went unnoticed across four copies.
    """
    blk = BEGIN + " -->\nbody\n" + END + "\n"
    ...
    print("splice-agents-block (graph-hooks) selftest OK")
    return 0
...
def main() -> int:
    ...
    ap.add_argument("--block", required=True, help="assets/agents-knowledge-graph.md")
    ...
    try:
        ...
        print(f"setup-graph-hooks: cannot read block asset: {e}", file=sys.stderr)
        return 1
    ...
    try:
        ...
        print(f"setup-graph-hooks: cannot read {a.file}: {e}", file=sys.stderr)
        return 1

    try:
        updated = splice(existing, block)
    except ValueError as e:
        print(f"setup-graph-hooks: {e}", file=sys.stderr)
        return 1

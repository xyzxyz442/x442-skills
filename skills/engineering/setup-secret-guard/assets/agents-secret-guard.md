...
- **A read inside an embedded shell is asked, not rewritten.** `docker run/exec`, `ssh`,
  `kubectl exec` and multi-line `-c '...'` scripts execute in another filesystem namespace where
  the host's `redact-view` does not exist, so no rewrite can both work and withhold the value.
  Reads of merely config-shaped paths there pass through untouched; credential-named ones prompt.
  Prefer reading on the host, or redact inside the guest.

...
mkdir -p "$T/cb"
printf '{"group":"g"}\n' > "$T/cb/none.json"
printf '{"boardPath":"../workspace/legacy"}\n' > "$T/cb/legacy.json"
printf '{"board":"../workspace/current","boardPath":"../workspace/stale"}\n' > "$T/cb/both.json"
printf 'not json' > "$T/cb/broken.json"
chk "a file naming no board is skipped" "../workspace/legacy" "$(handoff_config_board "$T/cb/none.json" "$T/cb/legacy.json")"
chk "board beats boardPath in one file" "../workspace/current" "$(handoff_config_board "$T/cb/both.json")"
chk "a missing or broken file is skipped, not fatal" "../workspace/current" "$(handoff_config_board "$T/cb/nope.json" "$T/cb/broken.json" "$T/cb/both.json")"
chk "nothing named reads as empty" "" "$(handoff_config_board "$T/cb/none.json")"

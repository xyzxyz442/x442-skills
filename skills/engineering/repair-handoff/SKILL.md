...
On a sectioned dedicated board every command must carry `HANDOFF_GROUP`. A command typed by hand
...
- **Index drift.** `INDEX.md` is generated by `handoff index`. Regenerate into a temp location and
  ...
  flag `INDEX.md` missing while docs exist. **The board root roll-up can legitimately lag now**: a
  lease commit only regenerates its own group's `INDEX-<group>.md`, so the roll-up is refreshed by
  `handoff index` or CI, and nothing treats that lag as a failure — the verifier does not check
  index freshness ([ADR 0019](../../../docs/adr/0019-a-lease-commit-touches-only-its-group-and-a-lost-race-retries.md)).
  A stale roll-up next to a current per-group index is that, not corruption; the fix is the same
  `handoff index` in step 4 either way.
- **Bundle table drift.** An orchestrator's `## Children` table is generated too, between its
  `handoff:children` markers. A bundle doc whose table disagrees with its children's real status —
  or that carries no markers at all, because it predates them — is the same class of drift; both
  are fixed by the same `handoff index` in step 4, which rewrites the block only when it actually
  changed.
...
- **Section resolution (dedicated boards only).** Confirm the board pointer in the tool hook commands
...
- **Brief repo identity (dedicated boards only).** `$BOARD/repos.json` is what `handoff export`
...
4. **Regenerate the index.** Safe to run unconditionally — it is generated output, and nobody's
   work is encoded in it. This also rebuilds every orchestrator's `## Children` table from its
   children's own frontmatter, adding the section to bundle docs that predate it:
   ```bash
   "$BOARD/handoff" index
   ```
